...
class PoiModel(ObjectItemModel, QObject):
    ...
    async def fetchPoiData(self):
        try:
            total, data = await self.thread_loop.coro_threadsafe(self._poi_fetch_api(self.city, self.keyword))
            self.rows = data
            self._total_result = data
            self.total = len(self._total_result)
            for i in range(1, total // 10 + 2):
                log.info(f"fetching page {i}")
                total, data = await self.thread_loop.coro_threadsafe(self._poi_fetch_api(self.city, self.keyword, i))
                log.info(f"page {i} fetched {total}")
                self._total_result.extend(data)
                self.total = len(self._total_result)
        except:
            traceback.print_exc()
        
    async def parse_baidu_poi(self, city, keyword, page = 1) -> Tuple[int, List[Poi]]:
